...
class HDCBC:
    ...
    def fit_predict_with_attribute(self,data_name,d,attribute_list,k):
    
        columns_to_keep = [col for col in d.columns if col not in attribute_list]

            # 重新赋值DataFrame，只保留需要的列
        data = d[columns_to_keep].values
            
        
        
        d['hdcbc_cluster'] = self.fit_predict(data_name,data)

        # 获取每个DBSCAN类的聚类中心（可以使用均值）
        cluster_centers = d.groupby('hdcbc_cluster').agg({
            'longitude': 'mean',
            'latitude': 'mean',
            'week_attr': 'mean',
            'hour': 'mean'
        ...
        # 标准化数据，以便更好地进行 K-means 聚类
        scaler = StandardScaler()
        scaled_centers = scaler.fit_transform(cluster_centers[['longitude', 'latitude', 'week_attr', 'hour']])

        # 使用 K-means 进行二次聚类（仅针对 DBSCAN 的中心点）
        kmeans = KMeans(n_clusters=k, random_state=42)
        kmeans.fit(scaled_centers)

        # 将 K-means 的分类结果映射回 DBSCAN 聚类中心
        cluster_centers['kmeans_cluster'] = kmeans.labels_

        # 将 K-means 结果回传到原始数据
        # 将原始数据点的 `hdcbc_cluster` 映射到聚类中心的 `kmeans_cluster`
        d['kmeans_cluster'] = d['hdcbc_cluster'].map(
            cluster_centers.set_index('hdcbc_cluster')['kmeans_cluster']
        )
        
        return d['kmeans_cluster'].values
...
if __name__ == "__main__":
    ...
    for key in data.keys():
        ...
        folder_path2=r'result/HDCBC/'+data_name
        ...
        labels=hdcb.fit_predict_with_attribute(data_name,data[key],attribute_list,k)
